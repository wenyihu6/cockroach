...
// TestEnsureLocalReadsOnGlobalTables ensures that all present time reads on
// GLOBAL tables don't incur a network hop even when there is a delay in the
// network latencies that exceed the previously hardcoded maxNetworkRTT (150ms).
func TestEnsureLocalReadsOnGlobalTablesWithDelay(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	skip.UnderDuress(t, "https://github.com/cockroachdb/cockroach/issues/102798")
	...
	// Enable the lead for global reads auto-tuning. Disabling it would make the
	// test fail since the default hardcoded lead time for global tables will be
	// too short for the 500ms network latencies.
