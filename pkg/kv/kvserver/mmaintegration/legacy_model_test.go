...
// (computeCPUCapacityWithCap). See TestComputeStoreCPURateCapacity for
// detailed scenarios showing where this model breaks down.
//
// It computes the CPU rate capacity for a single store given node-level CPU
// metrics. The model ensures that the mean store utilization (sum of store
// loads / sum of store capacities) equals the node-level CPU utilization, so
// that overload is detected at the store level exactly when it is detected at
// the node level. Individual stores may have different utilizations depending
// on their load.
//
// The two cases are:
//
...
//
// Furthermore, there are CPU consumers that we don't track on a per-replica
// (and thus per-store) level. Examples include RPC work, Go GC, SQL work etc.
// Some of the distributed SQL work could be happening because of a local
// replica, so ideally we should improve our instrumentation to track it per
// replica. Due to these gaps, we expect the NodeCPURateCapacity to be higher
// than StoresCPURate. So simply using NodeCPURateCapacity/NumStores as the
// per-store capacity would lead to over-utilization.
//
// Our approach is to apply the CPU utilization to StoresCPURate to compute a
// capacity and divide that evenly across stores. Specifically,
//
...
//
// i.e. we want to give the (collective) stores a capacity that results in the
// same utilization as reported at the process (node) level, i.e.
//
//	StoresCPURateCapacity = StoresCPURate/cpuUtil.
//
// Finally, we split StoresCPURateCapacity evenly across the stores.
//
// This construction ensures that there is overload as measured by node CPU
// usage exactly when there is overload as measured by mean store CPU usage:
//
...
// communication of node load to MMA. The NodeLoad.{ReportedCPU,CapacityCPU}
// is incrementally maintained as a sum of the load and capacity reported by
// each store (at different times).
//
// Additionally, when the meanCPUUtil indicates overload, at least one store
// will be above that mean, so it is overloaded as well and will induce load
// shedding.
//
// It's worth noting that this construction assumes that all load on the node
// is due to the stores. Take an extreme example in which there is a single
// store using up 1vcpu, but the node is fully utilized (at, say, 16 vcpus).
// In this case, the node will be at 100%, and we will assign a capacity of 1
// to the store, i.e. the store will also be at 100% utilization despite
// contributing only 1/16th of the node CPU utilization. The effect of the
// construction is that the store will take on responsibility for shedding
// load to compensate for auxiliary consumption of CPU, which is generally
// sensible.
//
// Panics if numStores <= 0 or nodeCPURateCapacity <= 0.
func computeStoreCPURateCapacityNaive(in storeCPURateCapacityInput) float64 {
	...
	if in.storesCPURate == 0 || cpuUtil < 0.01 {
		...
		return in.nodeCPURateCapacity / 2 / float64(in.numStores)
	}
