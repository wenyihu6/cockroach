...
//
// Note that MMARebalanceAdvisor should use the meansLoad summary constructed
// during its construction to compute the load summary for the provided
// candidate in IsInConflictWithMMA. The caller may modify its candidate set
// after calling this function. The caller is responsible for keeping track of
// the returned advisor and associating it with the corresponding existing
// store.
