...
func (bs *BufferedSender) sendBuffered(
	...
	if ok {
		...
		if log.V(5) && oldState == status.state && status.queueItems%100 == 0 {
			// Log every 100 events when in a stable state to track queue growth.
			log.KvExec.Infof(context.Background(), "buffered_sender: stream %d buffered %d events (state=%d, total_buffer_size=%d)",
				ev.StreamID, status.queueItems, status.state, bs.queueMu.buffer.len())
		}
