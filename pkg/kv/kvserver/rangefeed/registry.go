...
type Stream interface {
	...
	// Important to run in the background without blocking to avoid deadlock.
	// Promise to run when disconnected.
...
func (r *registration) setDisconnectedIfNotRMuLocked() (alreadyDisconnected bool) {
...
func (r *registration) setDisconnectedIfNot() {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.setDisconnectedIfNotRMuLocked()
}
...
func (r *registration) disconnect(pErr *kvpb.Error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if alreadyDisconnected := r.setDisconnectedIfNotRMuLocked(); !alreadyDisconnected {
