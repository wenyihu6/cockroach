...
func (r *Replica) closedTimestampTargetRLocked() hlc.Timestamp {
	notOverriden := closedts.LeadForGlobalReadsOverride.Get(&r.ClusterSettings().SV) == 0
	networkLatency := closedts.DefaultMaxNetworkRTT
	if autoTuningEnabled := notOverriden && closedts.LeadForGlobalReadsAutoTuneInterval.Get(&r.ClusterSettings().SV) != 0; autoTuningEnabled {
