...
func (m *mmaStoreRebalancer) rebalance(ctx context.Context, periodicCall bool) bool {
	diskOpts := allocatorimpl.MakeDiskCapacityOptions(&m.st.SV)
	m.mma.UpdateStoresStatuses(ctx, m.as.GetMMAStoreStatuses(), diskOpts.RebalanceToThreshold, diskOpts.ShedAndBlockAllThreshold)
