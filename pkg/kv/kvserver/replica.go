...
type mmaRangeMessageNeeded struct {
	...
	// field. All other fields are accessed only from replica.TryConstructMMARangeMsg
	// which is not called concurrently.
...
// populated with the most up-to-date state.
func (m *mmaRangeMessageNeeded) checkIfNeeded(
	...
	// state change.
	if !m.wasLeaseholder || m.stateChangeTriggered.Swap(false) {
		return true
	}
	...
	if len(m.lastLaggingStates) != len(progress) {
		return true
	}
