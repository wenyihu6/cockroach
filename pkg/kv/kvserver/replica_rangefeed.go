...
func (r *Replica) RangeFeed(
	...
	p, rangefeedCleanUpCallback, err := r.registerWithRangefeedRaftMuLocked(
	...
	return rangefeedCleanUpCallback, err
}
...
func (r *Replica) registerWithRangefeedRaftMuLocked(
...
) (rangefeed.Processor, func(), error) {
	...
	if p != nil {
		reg, filter, rangefeedCleanUpCallback := p.Register(span, startTS, catchUpIter, withDiff, withFiltering, withOmitRemote,
		...
		if reg {
			...
			catchUpIter = nil
			return p, rangefeedCleanUpCallback, nil
		}
	...
	// server shutdown.
	reg, filter, rangefeedCleanUpCallback := p.Register(span, startTS, catchUpIter, withDiff,
