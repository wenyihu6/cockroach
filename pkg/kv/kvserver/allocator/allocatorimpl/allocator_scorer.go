...
type ScorerOptions interface {
	...
	// will later be used to calculate the relative difference when comparing the range
	// count of the candidate stores. Lower range count means a better candidate to move the replica to.
...
// rangeCount has already been jittered in maybeJitterStoreStats. Return the
// provided rangeCount since ScatterScorerOptions uses jittered range count to
// scatter replicas across stores.
func (o *ScatterScorerOptions) adjustRangeCountForScoring(rangeCount int) int {
	return rangeCount
}
...
// maybeJitterStoreStats returns the default provided store list. ScatterScorerOptions is the only scorer option that
// jitters store stats to rebalance replicas across stores randomly.
