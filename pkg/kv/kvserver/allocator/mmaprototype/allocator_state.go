...
// its initialization during BuildMMARebalanceAdvisor.
//
...
func (a *allocatorState) IsInConflictWithMMA(
	...
	if advisor.disabled {
		log.KvDistribution.VEventf(ctx, 3, "MMA disabled: skipping conflict check for candidate s%d", cand)
		return false
	}
	...
	if cpuOnly {
		...
		if conflict {
			log.KvDistribution.VEventf(
				...
				cand, candSLS.dimSummary[CPURate], existingSLS.dimSummary[CPURate],
			)
		}
		return conflict
	}
	...
	if conflict {
		log.KvDistribution.VEventf(ctx, 2, "mma rejected candidate s%d: candidate=%v > existing=%v",
			cand, candSLS.sls, existingSLS.sls)
	}
	return conflict
}
