...
type loadAndCapacityMetrics struct {
	...
	// The metrics below populate the write bandwidth load as seen by MMA.
	// WriteBandwidth has UnknownCapacity, so no capacity or utilization metric.
	WriteBandwidthLoad *metric.Gauge
...
var (
	...
	metaWriteBandwidthLoad = metric.Metadata{
		Name:        "mma.store.write_bandwidth.load",
		Help:        "Write bandwidth load generated by ranges on this store",
...
func makeLoadAndCapacityMetrics() *loadAndCapacityMetrics {
	return &loadAndCapacityMetrics{
		...
		WriteBandwidthLoad: metric.NewGauge(metaWriteBandwidthLoad),
