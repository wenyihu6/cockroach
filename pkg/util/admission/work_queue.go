...
type WorkQueue struct {
	...
	mu struct {
		...
		// WorkInfo.Priority instead of WorkInfo.TenantID. Used in Resource Manager
		// mode to split work into foreground (priority >= NormalPri) and background
		// (priority < NormalPri) groups.
		//
		// This is a bool rather than a live read of the cluster setting so that
		// mode transitions can update this and all components have a consistent
		// view.
		//
...
func (q *WorkQueue) Admit(ctx context.Context, info WorkInfo) (AdmitResponse, error) {
	...
	var groupID uint64
	// TODO(wenyihu6): double check mode transition on useResourceGroup. cannot
	// transition inbetween.
	if q.mu.useResourceGroup {
		// TODO(wenyihu6): SQL should plumb resource group id to info
		groupID = priorityToResourceGroup(info.Priority)
	} else {
...
func (q *WorkQueue) getMaxCPULocked(groupID uint64) bool {
	...
	if q.mu.maxCPUGroups != nil {
		if fu, ok := q.mu.maxCPUGroups[groupID]; ok {
			return fu
		}
	}
	return false
...
func (q *WorkQueue) SetMaxCPUGroups(groups map[uint64]bool) {
	...
	for id, group := range q.mu.groups {
		...
		if group.cpuTimeBurstBucket.maxCPU != fu {
			prevQual := group.cpuTimeBurstBucket.burstQualification()
			group.cpuTimeBurstBucket.maxCPU = fu
...
// usePriorityBasedGroups, the resource group ID is derived from the work's
// priority (see priorityToResourceGroup).
