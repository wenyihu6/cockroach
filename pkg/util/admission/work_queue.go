...
type WorkQueue struct {
	...
	mu struct {
		...
		// for newly created groups (buckets init full). Only used if
...
// groupAggMetrics holds parent AggCounters for per-group admission
// metrics. One instance is shared across all groups within a
...
type workQueueOptions struct {
	...
	tiedToRange    bool
	usesAsyncAdmit bool
	// Per-group admission metrics. Only set when mode == usesCPUTimeTokens.
	perGroupAggMetrics *groupAggMetrics
...
func (q *WorkQueue) Admit(ctx context.Context, info WorkInfo) (AdmitResponse, error) {
	...
	if info.BypassAdmission {
		...
		if group.admittedCount != nil {
			group.admittedCount.Inc(1)
		}
		q.mu.Unlock()
	...
	if (len(q.mu.groupHeap) == 0 ||
		...
		q.adjustGroupUsedLocked(group, info.RequestedCount)
		// Save the child counter before releasing the mutex, since group
...
func (q *WorkQueue) AdmittedWorkDone(resp AdmitResponse, cpuTime time.Duration) {
	...
	func() {
		...
		// q.adjustGroupUsed(tenantID, resp.requestedCount) is called. Now that the
...
func (q *WorkQueue) granted(grantChainID grantChainID) int64 {
	...
	if group.admittedCount != nil {
		group.admittedCount.Inc(1)
		group.waitTimeNanos.Inc(waitDur.Nanoseconds())
	}
	if !isInGroupHeap(group) {
		q.mu.groupHeap.remove(group)
	}
...
func (q *WorkQueue) adjustGroupUsedLocked(group *groupInfo, delta int64) {
	...
	if q.mode == usesCPUTimeTokens {
		...
		// Track per-group token usage/returns via separate counters so
		// that rate() can be computed over arbitrary windows.
		if group.tokensUsed != nil {
			if delta > 0 {
				group.tokensUsed.Inc(delta)
			} else {
				group.tokensReturned.Inc(-delta)
			}
		}
	}
	if isInGroupHeap(group) {
...
func (q *WorkQueue) SetTenantWeights(groupWeights map[uint64]uint32) {
	...
	groupIDs := func() []uint64 {
		q.mu.Lock()
		defer q.mu.Unlock()
		tIDs := make([]uint64, len(q.mu.groups))
		i := 0
	...
	// groupIDs[index] represents the next group ID that needs to be updated.
	var index int
	n := len(groupIDs)
	...
	updateNextBatch := func() (repeat bool) {
		...
		for i := 0; i < batchSize; i++ {
			...
			id := groupIDs[index]
			gi := q.mu.groups[id]
			weight := q.getGroupWeightLocked(id)
...
type groupInfo struct {
	...
	// Per-group admission metric children. Only set when
...
func newGroupInfo(
	...
	*ti = groupInfo{
	...
	if aggMetrics != nil {
		...
		ti.waitTimeNanos = aggMetrics.waitTimeNanos.AddChild(tid)
		ti.tokensUsed = aggMetrics.tokensUsed.AddChild(tid)
		ti.tokensReturned = aggMetrics.tokensReturned.AddChild(tid)
	}
	return ti
}
...
func releaseGroupInfo(ti *groupInfo) {
	...
	if ti.admittedCount != nil {
		...
		ti.tokensUsed.Unlink()
		ti.tokensReturned.Unlink()
	}
