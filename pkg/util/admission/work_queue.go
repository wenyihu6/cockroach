...
type WorkQueue struct {
	...
	mu struct {
		...
		// composite groupKey (id + kind) so that the same uint64 ID
...
func (q *WorkQueue) gcGroupsResetUsedAndUpdateEstimators() {
	...
	for k, info := range q.mu.groups {
		// Skip GC for rg-kind containers whose IDs are configured in
		// rmGroups. Otherwise a GC + lazy re-create would leave the
		// group stuck at maxCPU=false defaults until the next
		// SetResourceGroupConfig or mode swap. With composite keys,
		// only rg-kind containers can match an rmGroups entry, so
		// tenant-kind containers (e.g., tenant 1) follow normal GC
		// even when their numeric ID matches an RG ID.
		configured := false
		if k.kind == rgKind {
			_, configured = q.mu.rmGroups[k.id]
		}
...
func (q *WorkQueue) applyResourceGroupConfigLocked() {
	...
	for id, g := range q.mu.rmGroups {
		...
		if !ok {
			group = newGroupInfo(k, weights[id], q.mode,
...
func (q *WorkQueue) refillRMGroupBurstBuckets(rate100, cap100 float64) {
	...
	for rgID, g := range q.mu.rmGroups {
		group, ok := q.mu.groups[rgGroupKey(rgID)]
		if !ok {
			continue
		}
