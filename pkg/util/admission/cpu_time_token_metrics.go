...
var (
	...
	// suffix is appended in makeCPUTimeTokenMetrics.
...
type cpuTimeTokenMetrics struct {
	...
	// mean rather than a histogram, also for cost reasons. Each tier gets
	// its own AggCounter because AggCounter.AddChild panics on duplicate
	// label values.
	...
	// token consumption and returns via adjustTenantUsedLocked.
