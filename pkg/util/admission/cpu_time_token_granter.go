...
// token bucket per burstQualification. Requests are only admitted (tryGet
...
// setting up a hierarchy of request types, where burstable requests can use
// more CPU than non-burstable requests.
//
// For example, on an 8 vCPU machine, it might be set up like this:
//
// - Burstable work -> 8 seconds of CPU time per second
// - Non-burstable work -> 6 seconds of CPU time per second
//
...
// buckets, resulting in a balance of (3, 1). Non-burstable work would
// now have to wait for its bucket to refill, while burstable work is
// still admissible.
//
// The immediate purpose of this is to achieve low goroutine scheduling
// latencies even in the case of a Serverless tenant sending a large workload
// to a multi-host cluster. The above rates will be set so as to limit CPU
// utilization to some cluster-setting-configurable maximum. For example, if
// the target max is 80%, and if the machine has 8 vCPUs, then the
// non-burstable work will be allowed 6.4 seconds of CPU time per second. In
// limiting CPU usage to some max, goroutine scheduling latency can be kept
// low.
//
...
type cpuTimeTokenGranter struct {
	...
	mu         struct {
		...
		// long as token bucket replenishing respects it also. See tryGrantLocked
		// for a situation where this invariant is relied on.
...
func (stg *cpuTimeTokenGranter) tryGrantLocked() bool {
	...
	if stg.mu.buckets[qual].tokens <= 0 {
		// The canBurst bucket always has more tokens than the noBurst
		// bucket (see the invariant on cpuTimeTokenGranter.mu.buckets).
		// So if the requested bucket has no tokens, neither does any
...
func (stg *cpuTimeTokenGranter) formatBuckets() string {
	...
	for qual := burstQualification(0); qual < numBurstQualifications; qual++ {
		fmt.Fprintf(&buf, " %s=%d",
			qual, stg.mu.buckets[qual].tokens)
	}
	buf.WriteRune('\n')
	return buf.String()
}
