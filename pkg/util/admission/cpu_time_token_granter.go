...
// token bucket per burstQualification level. Requests are only admitted
// (tryGet only returns true) if the bucket for the burst qualification of the
// request has positive tokens. Before a request is admitted, tokens are
// deducted from all buckets, not just the bucket that was checked initially.
// This enables setting up a hierarchy where some requests can use more CPU
// than others.
//
// For example, on an 8 vCPU machine, it might be set up like this:
//
// - canBurst work -> 8 seconds of CPU time per second (100%)
// - noBurst work  -> 6 seconds of CPU time per second (75%)
//
// A canBurst tenant's request for 5s would be admitted (canBurst bucket is
// positive). It would deduct from both buckets, resulting in (3, 1). noBurst
// work can still be admitted. After another 1s deduction: (2, 0). noBurst
// work must now wait, while canBurst work is still admissible.
//
// The immediate purpose is to achieve low goroutine scheduling latencies
// by limiting CPU utilization to a configurable maximum. For example, if the
// target max is 75%, and the machine has 8 vCPUs, then noBurst work will be
// allowed 6 seconds of CPU time per second.
//
...
type cpuTimeTokenGranter struct {
	...
	mu         struct {
		...
		//
		// Since admission deducts from all buckets, this invariant is true,
