...
type cpuTimeTokenFiller struct {
	...
	// GetKVWorkQueue via cpuTimeTokenGrantCoordinator. Queue
	// configuration and activeMode are both updated at the end of
	// resetInterval, after the refill is complete, so routing and
	// queue state change together.
...
// by that group's WEIGHT_CPU fraction before calling
...
func (s *rmStrategy) refillBurst(tokens tokenCounts, refillRates rates) {
	...
	// Normalize to the 100% CPU rate by dividing out canBurstTarget. This
	// relies on cpuTimeTokenLinearModel computing rates as
	// rate = cpuCapacity * time.Second * target / multiplier, so
	// rate / target is constant across targets. If the model becomes
	// non-linear, there are two alternatives:
	//
	//  1. Have fit return the full-CPU rate as a separate value instead of
	//     recovering it via division here.
	//  2. Set the second work queue's canBurst target to 1.0 so its refill
	//     rate is already at 100% CPU. However, this would violate granter
	//     Invariant #1 (lower-ordinal tiers must have more tokens), since
	//     tier 1 would then have a higher refill rate than tier 0.
	//
	// Option 1 is probably the right long-term fix, but we defer the
	// cleanup until the model actually changes.
...
// the new strategy is constructed for target computation and burst
// refill, but is only installed (along with queue configuration)
...
func (a *cpuTimeTokenAllocator) resetInterval(ctx context.Context) cpuTimeTokenMode {
	...
	targets := strategy.computeTargets(&a.settings.SV, burstDelta)
	...
	strategy.refillBurst(deltaRefillRates, a.refillRates)
	a.allocated = tokenCounts{}

	// Install the strategy and apply queue configuration after refill
...
func (a *cpuTimeTokenAllocator) configureQueue(strategy modeStrategy) {
	a.queues[0].setPriorityBasedGroups(
...
type workQueueIForAllocator interface {
	...
	refillBurstBucketForGroup(rgID uint64, toAdd int64, capacity int64, unscaledCapacity int64, maxCPU bool)
