...
func (a *cpuTimeTokenAllocator) allocateTokens(expectedRemainingTicksInInterval int64) {
	...
	for tier := 0; tier < a.numActiveTiers; tier++ {
...
func (a *cpuTimeTokenAllocator) resetInterval(ctx context.Context) cpuTimeTokenMode {
	...
	newRefillRates := a.model.fit(ctx, targets, a.numActiveTiers)
...
type cpuTimeModel interface {
	fit(ctx context.Context, targets targetUtilizations, numActiveTiers int) rates
}
...
func (m *cpuTimeTokenLinearModel) fit(
	ctx context.Context, targets targetUtilizations, numActiveTiers int,
) rates {
	if !m.init {
		m.init = true
	...
	if isLowCPUUtil {
		// Use the smallest target util across active tiers to compute
		// the upper bound. We must only scan active tiers to avoid
		// finding 0.0 from unused tier slots.
		smallestTargetUtil := math.MaxFloat64
		for tier := 0; tier < numActiveTiers; tier++ {
