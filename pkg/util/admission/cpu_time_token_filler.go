...
// KVCPUTimeUtilGoal is the non-burstable utilization goal for all work.
var KVCPUTimeUtilGoal = settings.RegisterFloatSetting(
	settings.SystemOnly,
	"admission.cpu_time_tokens.target_util",
	"the target CPU utilization for non-burstable work if using the KV CPU time "+
...
var KVCPUTimeUtilBurstDelta = settings.RegisterFloatSetting(
	...
	"the delta between non-burstable & burstable CPU utilization target if "+
		"using the KV CPU time token system, this delta is added to "+
		"admission.cpu_time_tokens.target_util to get the burstable target "+
...
// of CPU time per second. Then the refill rates for canBurst work would
...
// on the token buckets owned by cpuTimeTokenGranter; the TLDR is
...
type cpuTimeTokenAllocator struct {
	...
	// burst buckets that determine queue priority ordering.
...
// ones. The approach here satisfies this because the minimums are derived
// from the refill rates, which are ordered by priority.
//
...
// establishes during normal operation. One concrete benefit:
...
func (a *cpuTimeTokenAllocator) resetInterval(ctx context.Context) {
	// Compute target utilizations from cluster settings.
...
// more details.
//
// The buckets are arranged in a priority hierarchy. Higher priority buckets have
// higher target utilizations than lower priority buckets, and incoming requests
// generally require that the bucket for their priority has enough tokens, but then
...
// the token buckets owned by cpuTimeTokenGranter. fit returns the refill rates.
func (m *cpuTimeTokenLinearModel) fit(ctx context.Context, targets targetUtilizations) rates {
	...
	if isLowCPUUtil {
		...
		// of the low utilization. See the detailed analysis in the original
		// code for the math behind this.
		...
		// We clamp tokenToCPUTimeMultiplier to be in the interval [1, 20].
		...
		// than upward changes, since alpha is large in the case of the former.
		alpha := 0.5
