...
//
// rmStrategy does not cache per-group state. The per-group burstFrac
// values live on the long-lived groupInfo entries owned by WorkQueue,
// populated by the holder-driven apply path. refillBurst calls
// WorkQueue.refillRMGroupBurstBuckets each tick, which iterates
// q.mu.groups under one q.mu hold. This lets SetResourceGroupConfig
// apply derived state immediately under q.mu without needing to call
// back into the short-lived rmStrategy. See ResourceGroupConfigHolder's
// type comment for the storage rationale.
...
func (s *rmStrategy) refillBurst(tokens tokenCounts, refillRates rates) {
	...
	// rate / target is constant across targets and recovers the
	// 100% rate.
	//
	// TODO(wenyihu6): plumb the 100% CPU rate from the model directly
	// instead of recovering it here. Three reasons we haven't done it
	// yet:
	//   - cpuTimeModel.fit returns rates and is called from both
	//     resetInterval and (transitively) allocateTokens; widening
	//     the return type or adding an out-param ripples through
	//     every fit call site, the model interface, and the test
	//     mocks.
	//   - modeStrategy.refillBurst is implemented by both serverless
	//     and rm; serverless does not need rate100 so adding it as a
	//     refillBurst arg adds a parameter that one implementation
	//     ignores.
	//   - canBurstTarget is otherwise dead state on rmStrategy. The
	//     payoff of removing it is one division per refill tick on
	//     a 1ms cadence, which is not measurable. The cleanup is
	//     about clarity, not performance.
	// The right shape is probably for fit to return a small struct
	// (rates, rate100) so the per-tick division goes away and the
	// linear-model assumption stops leaking into the strategy. Worth
	// doing in the same change that touches the model for any other
	// reason.
	rate100 := float64(tokens[0][canBurst]) / s.canBurstTarget
	cap100 := float64(refillRates[0][canBurst]) / s.canBurstTarget
	// Hand off to WorkQueue, which iterates its groups under one q.mu
	// hold and refills each group's burst bucket using the per-group
	// burstFrac stored on groupInfo. Atomic across all groups; no
	// snapshot allocation per tick.
...
// sees groupInfo entries with populated burstFrac (the false→true
// transition in setUseResourceGroup snapshots the holder and applies
// the current derived config as a side effect; see
// WorkQueue.setUseResourceGroup).
func (a *cpuTimeTokenAllocator) resetInterval(ctx context.Context) cpuTimeTokenMode {
	...
	// group refill below sees groupInfo entries with populated
	// burstFrac (when going to RM). setUseResourceGroup on a
	// false→true transition snapshots the holder and applies the
	// current derived config synchronously, which materializes
	// per-group weight/maxCPU/burstFrac on groupInfos. Going the
	// other way (RM→serverless) just clears the flag; serverless
	// refill doesn't iterate per-group state.
	if modeChanged {
		a.configureQueue()
	}
...
// refill sees groupInfo entries with populated burstFrac.
//
...
// refill. Going from serverless to RM, this triggers an apply of the
// configHolder snapshot as a side effect of the false→true
// useResourceGroup transition (see WorkQueue.setUseResourceGroup), so
// the cycle's per-group refill below sees groupInfo entries with
// populated burstFrac.
//
...
type workQueueIForAllocator interface {
	...
	// false→true transition, it snapshots ResourceGroupConfigHolder
	// and applies the derived state on the WorkQueue (the only
	// mechanism for applying a config that was Set while in serverless
	// mode and for draining the constructor seed on first activation).
	// See WorkQueue.setUseResourceGroup for the contract.
