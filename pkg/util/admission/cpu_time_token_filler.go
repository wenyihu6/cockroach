...
func (a *cpuTimeTokenAllocator) resetInterval(ctx context.Context) cpuTimeTokenMode {
	...
	// offMode is skipped because it is not a real strategy - the
	// constructor defaults offMode to serverlessMode, and
	// GetKVWorkQueue handles the off case before reaching activeMode
	// routing.
	configChanged := false
...
// applies the current resource group config. offMode is not a valid
// argument - callers must guard against it (resetInterval skips the
// swap, the constructor maps off to serverless). Queue side effects
