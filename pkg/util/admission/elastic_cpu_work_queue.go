...
type elasticCPUInternalWorkQueue interface {
	...
	SetTenantWeights(groupWeights map[uint64]uint32)
...
func (e *ElasticCPUWorkQueue) SetTenantWeights(groupWeights map[uint64]uint32) {
	e.workQueue.SetTenantWeights(groupWeights)
}
