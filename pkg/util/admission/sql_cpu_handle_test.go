...
// meant to prevent. applyDerivedConfigLocked enforces this by always
// keying its writes via rgGroupKey.
func TestSetResourceGroupConfigRGOnly(t *testing.T) {
	...
	require.False(t, tenantMaxCPU,
		"tenant container must not inherit the rg-keyed maxCPU flag "+
			"(applyDerivedConfigLocked writes only rg-keyed entries)")
...
func TestSQLCPUHandleCloseAfterModeFlip(t *testing.T) {
	...
	// containers via applyDerivedConfigLocked, so the rg container
	// exists before Close runs.
	q.setUseResourceGroup(true)
