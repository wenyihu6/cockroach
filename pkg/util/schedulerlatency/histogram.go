...
// the Go runtime. It maintains both cumulative counts (for Prometheus export)
// and windowed counts (for TSDB quantiles). The code here is adapted from [1][2]:
//
...
type runtimeHistogram struct {
	...
	mu struct {
		...
		// Cumulative counts - these only ever increase, for Prometheus export.
		counts []uint64

		// Windowed counts for TSDB quantile calculations.
		// We maintain two windows (cur and prev) and merge them for WindowedSnapshot.
		windowedCur  []uint64
		windowedPrev []uint64
	}
...
// runtime/metrics.
//
// The windowDuration parameter controls the total duration of the windowed
// histogram (used for TSDB quantile calculations). The window is divided into
// two sub-windows that rotate at windowDuration/2 intervals.
func newRuntimeHistogram(
	...
	h.mu.counts = make([]uint64, numBuckets)
	h.mu.windowedCur = make([]uint64, numBuckets)
	h.mu.windowedPrev = make([]uint64, numBuckets)

	// Set up the ticker for window rotation.
	// We divide the window duration by 2 (WindowedHistogramWrapNum equivalent)
	// to maintain two sub-windows.
	...
	// Perform initial tick to set up the first window.
	h.windowed.Ticker.OnTick()

	return h
}

// rotate moves the current window to previous and clears current.
func (h *runtimeHistogram) rotate() {
	h.mu.Lock()
	defer h.mu.Unlock()
	// Move current to previous
	copy(h.mu.windowedPrev, h.mu.windowedCur)
	// Clear current
	for i := range h.mu.windowedCur {
		h.mu.windowedCur[i] = 0
	}
}

// update adds the histogram counts to both cumulative and windowed counts.
// The input histogram is expected to be a delta (interval) histogram with counts
// that should be added to the totals.
func (h *runtimeHistogram) update(his *metrics.Float64Histogram) {
	h.mu.Lock()
	defer h.mu.Unlock()
	counts, buckets := his.Counts, his.Buckets

	// Add to both cumulative and windowed counts.
	// This ensures Prometheus sees monotonically increasing cumulative counts,
	// while TSDB gets recent windowed data for meaningful quantiles.
	var j int
	for i, count := range counts { // add and reduce buckets
		h.mu.counts[j] += count
		h.mu.windowedCur[j] += count
...
// writeCumulative serializes the cumulative histogram state into the form
// prometheus expects.
func (h *runtimeHistogram) writeCumulative(out *prometheusgo.Metric) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.writeCountsLocked(h.mu.counts, out)
}

// writeWindowed serializes the windowed histogram state (cur + prev) into the
// form prometheus expects.
func (h *runtimeHistogram) writeWindowed(out *prometheusgo.Metric) {
	h.mu.Lock()
	defer h.mu.Unlock()

	// Merge cur and prev windows
	merged := make([]uint64, len(h.mu.windowedCur))
	for i := range merged {
		merged[i] = h.mu.windowedCur[i] + h.mu.windowedPrev[i]
	}
	h.writeCountsLocked(merged, out)
}

// writeCountsLocked writes the given counts to the prometheus metric.
// Caller must hold h.mu.Lock.
func (h *runtimeHistogram) writeCountsLocked(counts []uint64, out *prometheusgo.Metric) {
	sum := float64(0)
...
// Returns the cumulative histogram for Prometheus export.
...
// Returns cumulative counts since process start (monotonically increasing).
...
// Returns counts from the recent window (cur + prev) for meaningful quantiles.
func (h *runtimeHistogram) WindowedSnapshot() metric.HistogramSnapshot {
	// Maybe rotate window if needed
	tick.MaybeTick(h.windowed.Ticker)
